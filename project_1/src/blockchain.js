...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            if (self.height >= 0) {
                // If the chain already contains at least one block, link the new block to it
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            // Check if more than 5 minutes passed between the message creation and submission
            if((currentTime - msgTime) < 300) {
                // Verify the message/address/signature
    ...
    getBlockByHash(hash) {
        let self = this;
        return new Promise((resolve, reject) => {
            // Filter the chain by the given hash
    ...
    getBlockByHeight(height) {
        let self = this;
        return new Promise((resolve, reject) => {
            // Filter the chain by the given height
    ...
    getStarsByWalletAddress (address) {
        ...
        let stars = [];
        return new Promise((resolve, reject) => {
            // Filter the chain by the given address
            let blocks = self.chain.filter(p => p.getBData().address === address);
            let stars = [];
            blocks.forEach((item, i) => {
                stars.push({owner: address, star: item.getBData().star})
            });
